...
// 转为枚举
func ConvertColumnStatus(status model.ColumnStatus) sc_misc_api.ColumnStatus {
	return sc_misc_api.ColumnStatus(status)
}

// 领域模型转为传输模型
func ConvertColumnDTO(thing *model.Column) *sc_misc_api.ColumnDTO {
	if thing == nil {
		return nil
	}
...
// 数据库模型转换为领域模型
func ConvertColumnDTOs(things []*model.Column) []*sc_misc_api.ColumnDTO {
	if things == nil {
		return nil
	}
