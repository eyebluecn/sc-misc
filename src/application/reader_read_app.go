...
func (receiver ReaderReadApp) Login(ctx context.Context, username string, password string) (*model.Reader, error) {
	...
	if reader == nil {
		if username == "demo_reader" && password == "123456" {
			//测试用户手动创建。
			reader, err = domain.NewReaderDomainService().Create(ctx, &model.Reader{
				Username: username,
				Password: password,
			})
			if err != nil {
	...
	if reader == nil {
		...
		return nil, errs.CodeErrorf(enums.StatusCodeBadRequest, "用户名或者密码错误")
	}
	//密码错误
...
// 根据id来查询读者信息
func (receiver ReaderReadApp) QueryById(ctx context.Context, readerId int64) (*model.Reader, error) {
...
// 根据id来查询读者信息
func (receiver ReaderReadApp) QueryByIds(ctx context.Context, readerIds []int64) ([]*model.Reader, error) {
