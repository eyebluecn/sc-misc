...
func (r EditorReadApp) Login(ctx context.Context, username string, password string) (*model.Editor, error) {
	...
	if editor == nil {
		if username == "demo_editor" && password == "123456" {
			//测试用户手动创建。
			editor, err = domain.NewEditorDomainService().Create(ctx, &model.Editor{
				Username: username,
				Password: password,
			})
			if err != nil {
